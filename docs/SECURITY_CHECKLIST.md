...
### 追記：病院の複数ユーザー（招待コード）機能の権限チェック

`joinHospitalByInviteCode(userId, code)` / `regenerateInviteCode(userId)` を追加した際、
以下を確認・実装した（`tests/store.test.mjs`で保証）。

- 参加できるのは`role==="hospital"`かつ`refId`未設定（まだどこにも所属していない）ユーザーのみ。
  すでに病院に所属済みのユーザーが別の病院に乗り換えることはできない。
- 招待コードは8桁・紛らわしい文字（`0/O/1/I`）を除いた文字集合からランダム生成。
  病院ごとに1つのみ有効（総当たり成功率は低いが、本番では試行回数制限もRLS/RPC側で検討）。
- 招待コードの再発行は`refId`が自院に一致するユーザーのみ実行可能。再発行後は旧コードで
  参加できないことをテストで確認済み（コード漏洩時の失効手段として機能する）。
- 招待コード自体はチャット等の`esc()`対象ではない値（自由入力ではなく生成値の照合のみ）
