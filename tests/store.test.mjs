...
test("確定後のキャンセル：医師側からキャンセルすると募集が再公開され、他の医師が応募できる", async () => {
  const store = loadStore();
  const { api, getDB } = store;
  const doctor1 = await makeApprovedDoctor(store, "テスト医師J", "700010");
  const doctor2 = await makeApprovedDoctor(store, "テスト医師K", "700011");

  const poF = api.publishPosting("hp_1", {
    urgent:false, date:"2026-08-15", timeStart:"09:00", timeEnd:"12:00", overnight:false,
    type:"外来応援", cls:"b-gairai", department:"内科", pay:80000, lodging:"", ground:"", note:"",
  });
  const app1 = api.apply(doctor1, poF.id, { summary: "行程1" });
  const approve1 = api.approve("hp_1", app1.id);
  assert.ok(approve1.ok, approve1.err);

  const noReason = api.cancelAssignment("doctor", doctor1, approve1.id, "");
  assert.equal(noReason.ok, undefined);
  assert.match(noReason.err, /理由を入力してください/);

  const notParty = api.cancelAssignment("doctor", doctor2, approve1.id, "都合により");
  assert.equal(notParty.ok, undefined);
  assert.match(notParty.err, /当事者ではありません/);

  const cancel = api.cancelAssignment("doctor", doctor1, approve1.id, "急病のため");
  assert.ok(cancel.ok, cancel.err);

  const db = getDB();
  const asg = db.assignments.find(a => a.id === approve1.id);
  assert.equal(asg.status, "cancelled");
  assert.equal(asg.cancelReason, "急病のため");
  assert.equal(asg.cancelledBy, "doctor");
  const ap1After = db.applications.find(a => a.id === app1.id);
  assert.equal(ap1After.status, "cancelled");
  const postingFAfter = db.postings.find(p => p.id === poF.id);
  assert.equal(postingFAfter.status, "open");

  // 再公開されたので、別の医師が改めて応募できる
  const app2 = api.apply(doctor2, poF.id, { summary: "行程2" });
  assert.ok(app2.ok, app2.err);

  // 完了済み・すでにキャンセル済みの勤務は再度キャンセルできない
  const again = api.cancelAssignment("doctor", doctor1, approve1.id, "もう一度");
  assert.equal(again.ok, undefined);
  assert.match(again.err, /キャンセルできない状態です/);
});

test("確定後のキャンセル：病院側からもキャンセルでき、AuditLogに記録される", async () => {
  const store = loadStore();
  const { api, getDB } = store;
  const doctorId = await makeApprovedDoctor(store, "テスト医師L", "700012");

  const poG = api.publishPosting("hp_2", {
    urgent:false, date:"2026-08-16", timeStart:"09:00", timeEnd:"12:00", overnight:false,
    type:"外来応援", cls:"b-gairai", department:"内科", pay:80000, lodging:"", ground:"", note:"",
  });
  const app = api.apply(doctorId, poG.id, { summary: "行程1" });
  const approve = api.approve("hp_2", app.id);
  assert.ok(approve.ok, approve.err);

  const wrongHospital = api.cancelAssignment("hospital", "hp_1", approve.id, "他院からの誤操作");
  assert.equal(wrongHospital.ok, undefined);
  assert.match(wrongHospital.err, /当事者ではありません/);

  const cancel = api.cancelAssignment("hospital", "hp_2", approve.id, "医師の都合変更に伴う病院都合");
  assert.ok(cancel.ok, cancel.err);

  const db = getDB();
  const asg = db.assignments.find(a => a.id === approve.id);
  assert.equal(asg.status, "cancelled");
  assert.equal(asg.cancelledBy, "hospital");
  assert.ok(db.audit.some(a => a.action === "assignment.cancel" && a.detail.includes(approve.id)));
});
